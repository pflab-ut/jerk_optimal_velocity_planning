...
namespace osqp
{
    QPSolver::QPSolver(const OptimizerParam &param) : BaseSolver(param)
    {
        interface_.updateMaxIter(40000);
        interface_.updateRhoInterval(0);  // 0 means automoatic
        interface_.updateEpsRel(1.0e-4);  // def: 1.0e-4
        interface_.updateEpsAbs(1.0e-4);  // def: 1.0e-4
        interface_.updateVerbose(false);
    }
                         ...
                         OutputInfo& output)
    {
        ...
        const auto result = interface_.optimize(P, A, q, lower_bound, upper_bound);
        ...
        for(unsigned int i=0; i<N; ++i)
        {
            output.velocity[i] = std::sqrt(std::max(optval.at(i), 0.0));
            output.acceleration[i] = optval.at(i+N);
        }
